...
(() => {
  ...
  const checkpoints = {
    initial: { title: 'The first four examples', description: 'The model has two examples in each category. We ask the same test question at every stage.', fresh: ['pi01', 'pi02', 'rp01', 'rp02'] },
    after_teaching: { title: 'Six more examples later', description: 'The proof-integrity category now has eight examples. Runtime performance still has two.', fresh: ['pi03', 'pi04', 'pi05', 'pi06', 'pi07', 'pi08'] },
    after_expiry: { title: 'Two old examples are replaced', description: 'Examples nine and ten replace pi01 and pi02. The model now remembers pi03 through pi10.', fresh: ['pi09', 'pi10'] },
    after_restart: { title: 'After restarting the app', description: 'The app loads the saved model and asks the question again. It returns the same scores.', fresh: [] }
  };
  ...
  function renderSummary() {
    ...
    document.getElementById('run-completion').textContent = run.complete ? `${run.completed_operations}/${run.required_operations} steps completed` : `${run.completed_operations}/${run.required_operations} steps saved so far`;
    document.getElementById('run-summary').innerHTML = [
      metric('examples added', number(m.committed_updates)),
      metric('questions answered', number(m.answered_queries)),
      metric('proofs checked', number(m.proofs_verified)),
      metric('proof size', bytes(m.proof_bytes)),
      metric('total run time', seconds(run.worker_attempt_wall_seconds))
    ].join('');
    const a = document.createElement('a');
    a.href = run.source_url;
    a.textContent = 'Full results ↗';
  ...
  function memoryRow(label, snapshot, fresh) {
    ...
    return `<div class="memory-row"><div class="memory-label"><b>${escape(labels[label])}</b><span>${queue.length} / ${record.capacity} examples</span></div><ol class="slots" aria-label="${escape(labels[label])} examples, oldest first">${slots}</ol></div>`;
  }
  ...
  function renderCheckpoint() {
    ...
    const status = accepted ? 'Query completed' : 'Query not completed';
    let queryHTML = `<p class="eyebrow">The test question</p><p class="query-text">“${escape(record.query_text)}”</p>`;
    if (accepted) {
      const checks = [];
      ...
      if (laneCount && (receipt.checks.every_score_lane_equal || receipt.checks.every_signed_dot_lane_equal || receipt.checks.every_dot_lane_equal || receipt.checks.every_kernel_lane_equal || receipt.checks.every_lane_equal)) checks.push(`All ${laneCount} computed values match`);
      if (receipt.checks.exact_rational_ranking_equal) checks.push('Category order matches the direct calculation');
      if (receipt.checks.restarted_process_same_model_and_every_lane === true) checks.push('Same model and scores after restart');
      checks.push(`${number(receipt.metrics.proofs_verified)} proofs checked`);
      ...
      queryHTML += `<p class="prediction"><span>Answer</span>${escape(labels[receipt.prediction] || receipt.prediction)}</p><table class="score-table"><caption class="visually-hidden">Category scores: total similarity divided by number of examples</caption><tbody>${rows}</tbody></table><ul class="checks">${checks.map(check => `<li>${escape(check)}</li>`).join('')}</ul><p class="checkpoint-description">${escape(run.score_label)} · ${seconds(receipt.worker_seconds)} for this complete query · ${bytes(receipt.metrics.proof_bytes)} of proof data.</p>`;
    } else {
      queryHTML += '<p class="checkpoint-description">No query has finished at this stage. The examples shown come from the test sequence.</p>';
    }
  ...
  function renderText() {
    ...
    document.getElementById('selected-example').innerHTML = `<p class="text-id">${escape(example.id)} / ${escape(labels[example.label])} / teaching example</p><p>${escape(example.text)}</p>`;
  }
  ...
  fetch('public-record.json', { cache: 'no-cache' }).then(response => {
    ...
    select.replaceChildren(...record.runs.map(run => {
      const option = document.createElement('option'); option.value = run.id; option.textContent = run.id === 'linear-matched' ? 'New version · linear scoring' : 'Earlier version · squared scoring'; return option;
    }));
    select.value = runId;
    ...
    document.getElementById('checkpoint').innerHTML = '<p class="loading-message">The record could not be loaded. <a href="public-record.json">Download the results</a> or use the links below.</p>';
  });
})();
